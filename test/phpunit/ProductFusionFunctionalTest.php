...
/**
 * \file    test/phpunit/ProductFusionFunctionalTest.php
...
/**
 * Class ProductFusionFunctionalTest
 *
 ...
 *
 * @package Testproductfusion
 */
class ProductFusionFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modProductFusion')->click();
		} else {
			// Disable the module
			$this->byHref('modProductFusion')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/productfusion/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('productfusion/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/productfusion/admin/about.php');
		$this->authenticate();
		return $this->assertContains('productfusion/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('productfusionwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modProductFusion_ProductFusionTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modProductFusion_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
	...
	/**
	 * Test merging products independant
	 *
	 * @depends testModuleEnabled
	 * @return bool
	 */

	public function testProductMerge()
	{
		// Créer deux produits
		$product1Id = $this->createProduct("Product1", "Ref1", 10.0);
		$product2Id = $this->createProduct("Product2", "Ref2", 20.0);

		// Fusionner les deux produits
		$this->mergeProducts($product1Id, $product2Id);

		// Vérifier que le produit fusionné existe et que le produit archivé a été supprimé
		$isMergedProductExist = $this->isProductExist("Product1 - Product2", "Ref1 - Ref2", 30.0);
		$isArchivedProductExist = $this->isProductExist("Product2", "Ref2", 20.0);

		$this->assertTrue($isMergedProductExist, "Le produit fusionné existe");
		$this->assertFalse($isArchivedProductExist, "Le produit archivé a été supprimé");
	 ...
	 */
	private function createProduct($label, $ref, $price)
	{
		// TODO: Implémentez la logique pour créer un produit et renvoyez son ID
	}

	/**
	 * Helper function to merge two products
	 *
	 * @param int $productId1 First product ID
	 * @param int $productId2 Second product ID
	 * @return void
	 */
	private function mergeProducts($productId1, $productId2)
	{
		// TODO: Implémentez la logique pour fusionner deux produits
	}
	 ...
	 */
	private function isProductExist($ref)
	{
		$product = new Product($this->db);
		$result = $product->fetch('', $ref);
		...
		if ($result > 0) {
			...
			throw new Exception("Product fetch error: " . $product->error);
		}
	}
